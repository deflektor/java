...
public class Document {
	...
	public void processDocument() throws Exception
	{
		...
		wiDoc = wiRepEngine.openDocument(RetriveDocument());
		
		myDatabase = new DocumentOracle(my_Database, my_DB_user, my_DB_password);
		
		processProperties(wiDoc.getProperties());
	...
	private void processSQLDP (SQLDataProvider my_sqldp)
	{
	...
	private void processSQLDataProvider (SQLDataProvider my_sqldp)
	{
		SQLDataProvider wiSDP = my_sqldp;
	   	...
	   	String DPpartialrefresh = wiSDP.isPartialResult()?"1":"0";
   	 
	   	String DProws = ((Integer)wiSDP.getRowCount()).toString();
	   	String DPnbflow = ((Integer)wiSDP.getFlowCount()).toString();
	   	String DPrefreshable = wiSDP.isIgnoredForRefresh()?"0":"1";
        ...
        } else 
        {
	   	...
	   	try {
			//myDatabase.insertBoxiDataProvider()
	   		myDatabase.insertBoxiDataProvider(my_Report_name, DPName, "1", DPrefreshable, DPDate, ((Integer)wiSDP.getDuration()).toString(), "1",
	   				DPnbflow, DProws, DPpartialrefresh, DPUniverse, sql_output);
		} catch (Exception e)
		{
	...
	public static void main(String[] args) {
	      if (args.length != 8)
	      {
		      ...
		      System.out.println("----------------------------------------------------------------------------");
		      System.out.println("usage: java -jar document.jar <system> <user> <password> <authent> <doc_id> <database> <db_user> <db_password>");
		      System.out.println(" ");
