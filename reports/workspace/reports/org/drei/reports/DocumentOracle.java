...
public class DocumentOracle{
	...
	DocumentOracle(String db, String user, String password) throws Exception
	{
		...
		conn = DriverManager.getConnection("jdbc:oracle:thin:"+my_user+"/"+my_passwd+"@//crs020.it.internal:1521/"+my_db);
	}
	...
	public boolean insertBoxiDataProvider(String reportname, String dpname, 
			int i, int j, Calendar lastexecutiontime,
			int k, int maxnblines, int nbcubes, int l,
			int m, String universename, String stmtsql) throws Exception
	{
		boolean ret_code = false;
		...
		String mergeSql = "MERGE INTO admin.boxi_dataprovider b"
				...
				+ "         b.stmt_sql_clob=e.stmt_sql_clob,"
				+ "         b.update_date=e.update_date"
				+ " WHEN NOT MATCHED THEN"
				+ "  INSERT (b.report_name,"
				+ "          b.dataprovider_name,"
		...
		prep.setInt   (6, k);
		prep.setInt   (7, maxnblines);
		prep.setInt   (8, nbcubes);
		prep.setInt   (9, l);
		prep.setInt   (10, m);
		prep.setString(11, universename);
		prep.setString(12, stmtsql.substring(0, stmtsql.length()<3900 ? stmtsql.length() : 3900));
		prep.setClob  (13, newClob);
		prep.setString(14, getCurrentDate());
		ret_code = prep.execute();
